...
using namespace std;
/* This program implements the linear probing hash table with given hash functions. */
...
LinearProbingHashTable::LinearProbingHashTable(HashFunction<std::string> hashFn) {
    ...
    /* Initializes the elements in the array to empty. */
...
/* This function looks for an element and determines its position.
 * If found, it returns the index of the element in the table;
 * if not, it returns -1. */
int LinearProbingHashTable::findElement (const string& elem) const {
    ...
    int code = Hash(elem);
    for (int i = 0; i < allocatedSize; i++ ) {
        //Only looks at an item's value if its type is FILLED.
        if (elems[(code+i) % allocatedSize].type == SlotType::FILLED) {
            if (elems[(code+i) % allocatedSize].value == elem) {
                return (code+i) % allocatedSize;
            }
        }
        if (elems[(code+i) % allocatedSize].type == SlotType::EMPTY){
            return -1;
        }
    }
    return -1;
...
bool LinearProbingHashTable::insert(const string& elem) {
    ...
    int code = Hash(elem);
    for (int i = 0; i < allocatedSize; i++ ) {
        /* Adds the element to this slot if it's empty or a tombstone. */
        if (elems[(code+i) % allocatedSize].type != SlotType::FILLED) {
            elems[(code+i) % allocatedSize].value = elem;
            elems[(code+i) % allocatedSize].type = SlotType::FILLED;
            logicalSize ++;
            return true;
        }
    }
...
bool LinearProbingHashTable::remove(const string& elem) {
    ...
    /* If the hash table contains this element, it accesses it by its index position and marks it as a tombstone. */
    if (index != -1) {
