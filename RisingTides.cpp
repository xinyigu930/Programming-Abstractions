...
Grid<bool> floodedRegionsIn(const Grid<double>& terrain,
    ...
    for (const GridLocation& elem : sources){
    ...
    while (!queue.isEmpty()) {
            ...
            for (int i = 0; i < 4; i++) {
                int tempX = loc.row + neighbors[i];
                int tempY = loc.col + neighbors[i+1];
                if (!water.inBounds(tempX, tempY)) continue;
                if (terrain.get(tempX, tempY) <= height && water[{tempX, tempY}] == false){
                    water.set({tempX, tempY}, true);
...
STUDENT_TEST("The initial water level is below the terrain."){
    Grid<double> world = {
        { 4, 2, 4, 2 },
        { 3, 6, 2, 7 },
        { 9, 3, 3, 5 }
    ...
    /*  The water doesn’t flood anything, including its initial terrain cell.  */
...
STUDENT_TEST("Handles one row terrain."){
    Grid<double> world = {
        { 3, 2, 1, 2, 4, 5, 1, 10}
    };
    Vector<GridLocation> sources = {
    ...
    /*  The water flows left and right .  */
    Grid<bool> water = floodedRegionsIn(world, sources, 4.0);
    Grid<bool> expected = {
...
STUDENT_TEST("Handles one column terrain."){
    Grid<double> world = {
        { 1 },
        { 2 },
        { 3 },
